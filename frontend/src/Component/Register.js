...
class Register extends Component {
  ...
  onSubmit(e) {
    ...
    let post_valid = this.state.add_post ? (validateForm(this.onPost.current.state.errors)): true;
     if (validateForm(this.state.errors) && post_valid) {
         register(newUser).then(res => {
             if (res.res == 'Created') {
                 if (this.state.add_post){
                    this.onPost.current.onAddPost(res.id).then( res => {
                    if (res == 'Created') {
          ...
          <div className="col-md-6 mt-5 mx-auto">
            <form noValidate onSubmit={this.onSubmit}>
              ...
              <div className="form-group">
                ...
                {this.state.add_post ? <Post ref={this.onPost}/> : <br/>}
              <button
                type="submit"
                className="btn btn-lg btn-primary btn-block"
              >
